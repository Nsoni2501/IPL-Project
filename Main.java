...
public class Main {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        ArrayList<HashMap<String,String>> matchesList = convertCSVToMap("./assets/matches.csv");
...
//  2. Number of matches won of all teams over all the years of IPL.
        matchesWon(matchesList);
//  3. For the year 2016 get the extra runs conceded per team.
        extrasConceded(deliveriesList);
//  4. For the year 2015 get the top economical bowlers.
//  5. Create your own scenario (Number of matches played in each city)
        citiesList(matchesList);
    }

    private static void citiesList(ArrayList<HashMap<String, String>> matchesList) {
    ...
    private static void extrasConceded(ArrayList<HashMap<String, String>> deliveriesList) {
        System.out.println("3. For the year 2016 get the extra runs conceded per team.");
        long extras = deliveriesList.stream().filter(p -> Integer.parseInt(p.get("match_id"))>=577 && Integer.parseInt(p.get("match_id"))<=636 && Integer.parseInt(p.get("extra_runs"))>0).map(p -> p.get("extra_runs")).count();
        System.out.println(extras);
    }

    private static void matchesWon(ArrayList<HashMap<String, String>> matchesList) {
    ...
    private static void matchesPlayed(ArrayList<HashMap<String, String>> matchesList) {
